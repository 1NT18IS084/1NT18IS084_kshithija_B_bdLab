...
//Total number of people who referred movies
...
public class Positivefeedback {
	...
	public void map(LongWritable key, Text value, OutputCollector<Text, IntWritable> output, Reporter reporter) throws IOException {
		...
		if(tokens[2].matches("Suspense")) {
			output.collect(new Text("Review"),new IntWritable (Integer.parseInt(tokens[3])));
		}
		}
	}
	...
	public void reduce(Text key, Iterator<IntWritable> values, OutputCollector<Text, IntWritable> output, Reporter reporter) throws IOException { 
			...
			output.collect(new Text("Feedback are:"), new IntWritable(transcount));
		}
	}
	...
	public static void main(String[] args) throws Exception {
		...
		JobConf conf = new JobConf(Positivefeedback.class);
